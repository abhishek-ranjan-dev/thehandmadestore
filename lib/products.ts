...
export type Product = {
  ...
  dateAdded: string;
  /**
   * The primary handcrafted material, surfaced as a small badge on product
   * cards (e.g. "Cork", "Seed Paper", "Khadi"). Optional so the CMS can leave
   * it blank; the storefront hides the badge when it's absent.
   */
  material?: string;
};

/** Look up a product by id — used by the cart & wishlist screens to resolve
 *  their stored ids back to full product records. */
